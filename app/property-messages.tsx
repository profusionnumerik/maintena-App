/**
 * Messagerie bailleur ↔ locataire (côté bailleur)
 * Reçoit propertyId en query param : /property-messages?propertyId=xxx
 * Logique identique à (tenant)/messages.tsx — expo-audio SDK 54
 */
      ...
      <Pressable style={[ab.bubble, isMine ? ab.mine : ab.them]} onPress={toggle}>
        <View style={ab.inner}>
          <View style={[ab.playBtn, isMine ? ab.playMine : ab.playThem]}>
            <Ionicons name={status.playing ? "pause" : "play"} size={14} color={isMine ? COLORS.teal : "#fff"} />
          </View>
          <View style={{ flex: 1, gap: 4 }}>
            <View style={ab.track}><View style={[ab.fill, { width: `${Math.round(pos * 100)}%` }]} /></View>
            <Text style={[ab.dur, isMine ? ab.durMine : ab.durThem]}>{duration > 0 ? fmtDur(duration) : "…"}</Text>
          </View>
...
function TextBubble({ msg, isMine }: { msg: ChatMessage; isMine: boolean }) {
  ...
  return (
    <View style={[tb.wrapper, isMine ? tb.right : tb.left]}>
      {!isMine && <View style={tb.avatar}><Ionicons name="person-outline" size={11} color={COLORS.teal} /></View>}
...
function MicButton({ onAudioReady, disabled }: { onAudioReady: (uri: string, durationMs: number) => void; disabled?: boolean }) {
  ...
  const start = async () => {
    ...
    } catch (e) { console.warn("[mic] start", e); Alert.alert("Erreur", "Impossible de démarrer l'enregistrement."); }
  };
  ...
  return (
    <Pressable style={[mc.btn, isRecording && mc.btnActive, disabled && mc.btnDim]} onPressIn={start} onPressOut={stop}>
      <Ionicons name={isRecording ? "stop-circle" : "mic-outline"} size={18} color={isRecording ? "#EF4444" : (disabled ? "rgba(255,255,255,0.3)" : "rgba(255,255,255,0.8)")} />
    </Pressable>
  );
}
...
export default function LandlordMessages() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(
      ...
      (err) => { console.warn("[landlord/messages]", err); setLoading(false); }
    );
  }, [propertyId]);
  ...
  const sendText = useCallback(async () => {
    const t = text.trim();
    if (!t || !propertyId || !user?.uid || sending) return;
    setText("");
    setSending(true);
    try {
  ...
  }, [text, propertyId, user?.uid, sending]);
  ...
  const sendAudio = useCallback(async (uri: string, durationMs: number) => {
    ...
    try {
      ...
      const ref  = storageRef(storage, `messages/${propertyId}/${id}.m4a`);
      ...
      console.warn("[landlord/messages] sendAudio", e);
      ...
      >
        ...
        <View style={[s.header, { paddingTop }]}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={s.subtitle}>Locataire</Text>
          </View>
        ...
        {loading ? (
          ...
          <View style={s.center}>
            ...
            <Text style={s.emptyDesc}>Envoyez un message texte ou maintenez 🎤 pour un message vocal.</Text>
          </View>
        ) : (
          <FlatList
            ref={flatRef}
        ...
        <View style={[s.bar, { paddingBottom: paddingBot }]}>
          <TextInput
            ...
            blurOnSubmit={false}
          />
          {!hasText && <MicButton onAudioReady={sendAudio} disabled={sending} />}
          {hasText && (
            <Pressable style={[s.sendBtn, sending && s.sendBtnDim]} onPress={sendText}>
...
const s = StyleSheet.create({
  header:     { flexDirection: "row", alignItems: "center", gap: 12, paddingHorizontal: 20, paddingBottom: 14, borderBottomWidth: 1, borderBottomColor: "rgba(255,255,255,0.07)" },
  ...
  onlineDot:  { width: 10, height: 10, borderRadius: 5, backgroundColor: "#10B981", borderWidth: 2, borderColor: COLORS.dark },
  ...
  bar:        { flexDirection: "row", alignItems: "flex-end", gap: 10, paddingHorizontal: 14, paddingTop: 10, borderTopWidth: 1, borderTopColor: "rgba(255,255,255,0.08)", backgroundColor: "rgba(0,0,0,0.18)" },
  input:      { flex: 1, backgroundColor: "rgba(255,255,255,0.08)", borderRadius: 22, borderWidth: 1, borderColor: "rgba(255,255,255,0.12)", paddingHorizontal: 16, paddingTop: 10, paddingBottom: 10, fontSize: 14, fontFamily: "Inter_400Regular", color: "#fff", maxHeight: 120, minHeight: 40 },
